...
layui.define(["http", "getFn"], function(e) {
	...
	carousel.on('change(carousel)', (obj) => {
		carIdex = obj.index;
		getEchartsFn();
	});
				...
				for (var i = 0; i < country.length; i++) {
					...
					if (is == 1) {
						checks += '<div class="layui-inline">' +
							...
							'" lay-skin="primary" lay-filter="check" title="' +
							title + '" checked />' +
							'</div>' +
							'</div>';
						checkArr.push(id);
	...
	*/
	...
	var siteEl = $("#homeEl").val(),
		siteTimeout = null,
		siteType = $("div.lay_type").eq(0).html();
				...
				if (stationId > -1) {
					http({
						...
						success: function(res) {
							...
							var content = type < 0 ? '../pages/layHome.html?id=' +
								stationId : '../pages/layHomes.html?id=' +
								stationId;
							layer.open({
								type: 2,
								shade: 0,
